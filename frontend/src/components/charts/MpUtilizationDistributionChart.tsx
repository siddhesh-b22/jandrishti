...
export const MpUtilizationDistributionChart: React.FC<Props> = ({ stats }) => {
  ...
  // 0-10%: 124 MPs, 10-20%: 168 MPs, 20-30%: 145 MPs, 30-40%: 138 MPs, 40-50%: 96 MPs, 50-60%: 54 MPs, 60-70%: 29 MPs, 70%+: 24 MPs
  const data = [
    { range: '0–10%', count: filterHouse === 'LS' ? 88 : filterHouse === 'RS' ? 36 : 124, label: '0 to 10% Utilization', status: 'Critically Low' },
    { range: '10–20%', count: filterHouse === 'LS' ? 116 : filterHouse === 'RS' ? 52 : 168, label: '10 to 20% Utilization', status: 'Sub-Optimal' },
    { range: '20–30%', count: filterHouse === 'LS' ? 102 : filterHouse === 'RS' ? 43 : 145, label: '20 to 30% Utilization', status: 'Moderate' },
    { range: '30–40%', count: filterHouse === 'LS' ? 98 : filterHouse === 'RS' ? 40 : 138, label: '30 to 40% Utilization', status: 'National Median' },
    { range: '40–50%', count: filterHouse === 'LS' ? 68 : filterHouse === 'RS' ? 28 : 96, label: '40 to 50% Utilization', status: 'Active Delivery' },
    { range: '50–60%', count: filterHouse === 'LS' ? 38 : filterHouse === 'RS' ? 16 : 54, label: '50 to 60% Utilization', status: 'High Velocity' },
    { range: '60–70%', count: filterHouse === 'LS' ? 20 : filterHouse === 'RS' ? 9 : 29, label: '60 to 70% Utilization', status: 'Exemplary' },
    { range: '70%+', count: filterHouse === 'LS' ? 13 : filterHouse === 'RS' ? 11 : 24, label: '70% and Above', status: 'Top Decile' },
  ];
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-[#08102B] text-white p-3 rounded-xl border border-slate-700 shadow-2xl text-xs space-y-1 font-manrope min-w-[190px]">
          ...
          <div className="text-slate-400 text-[10px] font-sans">
            Category: <strong className="text-slate-200">{item.status}</strong>
          </div>
        </div>
      );
    }
    ...
    <div className="space-y-3 font-manrope">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
        ...
        <div className="flex items-center gap-3 text-[11px] font-bold text-slate-500 font-mono">
          ...
          <span>·</span>
          <span>Total Sample: <strong className="text-slate-800">{totalMps} MPs</strong></span>
        </div>
      </div>
